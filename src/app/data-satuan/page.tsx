...
const Page = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            console.log(err); // Menambahkan logging untuk error
    ...
    const handleModalsUpdate = async (id : string | number | undefined) => {
        ...
        try {
            const response = await fetch(`/api/satuan-barang?id=${id}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
    ...
    const handleAddData = async () => {
        if (!addData.nama.trim()) {
            showDialog('error', "Gagal", "Nama satuan Barang harus diisi");
            return;
        }
        ...
        <LayoutAdmin>
            <div className='flex flex-col gap-10 w-full h-max'>
                <Breadcrumbs 
                    ...
                    title="Master Data"
                    link={[
                        {title : "Data Satuan", link : "/data-satuan"}
                    ]}
                />
